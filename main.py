...
class Order(StatesGroup):
    ...
    payment = State()
    trxid = State()
    # For Crunchyroll: extra WhatsApp number
    extra_input = State()
...
def get_expiry_date(days):
    """Return a formatted expiry date from now + days"""
...
# ─── KEYBOARDS (2-COLUMN CLEAN UI) ───
def main_menu(uid):
    """2-column category layout – Medium sized buttons side by side"""
...
async def order_start(call: CallbackQuery, state: FSMContext):
    ...
    # YouTube, Netflix, Crunchyroll: ask for email (gmail)
    ...
    # Crunchyroll: special flow – first ask payment, then details
...
# ─── INPUT HANDLER (for email / location) ───
...
async def get_input(msg: Message, state: FSMContext):
    ...
    # For Crunchyroll, after email, ask for WhatsApp number
    ...
    # Other categories: go to payment
    lines = [
        f"📦 *{prod['name']}*",
        f"📧/📍 তথ্য: {text}",
        f"💰 মূল্য: {fmt(prod['price'])}",
...
# ─── Crunchyroll extra input (WhatsApp number) ───
...
async def process_payment(call_or_msg, state: FSMContext, pmethod, trx):
    ...
    price = prod["price"]
    expiry_days = prod.get("expiry_days", 30)
    ...
    # VPN/Proxy: Auto deliver if stock available
    if cat_id in ["vpn", "proxy"]:
        stock = db.get_available_stock(prod["id"])
        if stock:
            # Use stock's expiry_days if available, else product's
            stock_expiry = stock.get("expiry_days", expiry_days)
            delivery_data = {"server": uinput or "Auto", "expires": f"{stock_expiry} days"}
            ...
            delivery_text += f"⏰ Expiration: {stock_expiry} Days\n"
            delivery_text += f"📅 Expiry Date: {get_expiry_date(stock_expiry)}"
    ...
    # YouTube / Netflix / Crunchyroll: pending for admin approval
    else:
        db.update_order(oid, "pending")
        lines = [
...
async def approve_order(call: CallbackQuery):
    ...
    # Try auto-deliver from stock for VPN/Proxy
    if prod and prod["category_id"] in ["vpn", "proxy"]:
        ...
        if stock:
            ...
            delivery_data["expiry_date"] = get_expiry_date(stock_expiry)
            db.update_order(oid, "delivered", delivery_data)
            
            full_text = f"✅ *আপনার YouTube প্রিমিয়াম একটিভ হয়েছে!*\n\n{user_text}"
            full_text += f"🌍 Server: {order.get('user_input', 'Auto')}\n"
            full_text += f"⏰ মেয়াদ: {stock_expiry} দিন\n"
            full_text += f"📅 এক্সপায়ার: {get_expiry_date(stock_expiry)}\n"
            full_text += f"✅ স্ট্যাটাস: *Active*"
            
            try:
                await bot.send_message(order["user_id"], full_text, parse_mode="Markdown")
            except:
                pass
        else:
            db.update_order(oid, "delivered")
            try:
                await bot.send_message(order["user_id"], f"✅ *আপনার অর্ডার #{oid} একটিভ করা হয়েছে!*", parse_mode="Markdown")
            except:
                pass
        lines = [f"✅ *Order #{oid} Approved with auto-delivery!*"]
        await call.message.edit_text("\n".join(lines), reply_markup=admin_kb(), parse_mode="Markdown")
        return
    
    # For YouTube, Netflix, Crunchyroll
    db.update_order(oid, "delivered")
    
    # Send detailed delivery message
    now = datetime.now()
    expiry_date = now + timedelta(days=expiry_days)
    
    user_text = (
        f"✅ *আপনার অর্ডার কনফার্ম হয়েছে!*\n\n"
        f"📦 *{order['product_name']}*\n"
        f"🆔 Order ID: #{oid}\n"
        f"📧 Provided Info: {order.get('user_input', 'N/A')}\n\n"
        f"📅 একটিভ হয়েছে: {now.strftime('%d %B %Y %I:%M %p')}\n"
        f"⏰ মেয়াদ: {expiry_days} দিন\n"
        f"📅 এক্সপায়ার: {expiry_date.strftime('%d %B %Y')}\n"
        f"✅ স্ট্যাটাস: *Active*\n\n"
        f"🙏 ধন্যবাদ আমাদের সাথে ব্যবসা করার জন্য! @FBSKYSUPPORT"
    )
...
async def addbal_amt(msg: Message, state: FSMContext):
    try:
        ...
        try:
            await bot.send_message(uid, f"🎉 *আপনার ওয়ালেটে {fmt(amt)} যুক্ত করা হয়েছে!*\nবর্তমান ব্যালেন্স: {fmt(new_bal)}", parse_mode="Markdown")
        except:
            pass
    except:
...
async def deliver_file(msg: Message, state: FSMContext):
    ...
    user_text = f"✅ *আপনার অর্ডার #{oid} ডেলিভারি করা হয়েছে!*\n\n"
...
async def addcat_start(call: CallbackQuery, state: FSMContext):
    ...
    lines = [
        "➕ *Add New Category*",
        "",
        "ক্যাটাগরি আইডি লিখুন (স্পেস ছাড়া):",
        "উদাহরণ: `spotify_prem`",
    ]
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text="🔙 Back", callback_data="admin_cats"))
...
async def addprod_start(call: CallbackQuery, state: FSMContext):
    ...
    lines = [
        "➕ *Add Product*",
        "",
        "প্রোডাক্ট এর ID দিন (স্পেস ছাড়া):",
        "উদাহরণ: `vpn_nord_1m`",
    ]
    kb = InlineKeyboardBuilder()
    ...
    try:
        ...
        if cat_id in ["vpn", "proxy"]:
            ...
            db.add_product(data["addprod_id"], cat_id, data["addprod_name"], 
                          data["addprod_price"], data["addprod_bonus"], stock_type, expiry)
...
async def editprod_select(call: CallbackQuery, state: FSMContext):
    ...
    lines = [
        ...
        f"ID: `{pid}`",
        f"💰 মূল্য: {fmt(prod['price'])}",
        f"⏰ মেয়াদ: {prod.get('expiry_days', 30)} Days",
        "",
