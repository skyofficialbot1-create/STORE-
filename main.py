...
async def admin_callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Approve/Reject Order
        ...
        with db() as conn:
            ...
            if not order:
                await query.message.edit_text(f"⚠️ অর্ডার {ord_id} আগেই সম্পন্ন বা বাতিল করা হয়েছে।")
                return
    ...
    # Approve/Reject Deposit (Add Money)
        ...
        with db() as conn:
            ...
            if not dep:
                await query.message.edit_text(f"⚠️ রিকোয়েস্ট {req_id} আগেই প্রসেস করা হয়েছে।")
                return
...
async def start_db_restore_process(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_row = get_user(str(update.effective_user.id))
    if not is_admin(user_row):
        await query.message.reply_text("❌ অ্যাক্সেস ডিনাইড!")
        return ConversationHandler.END
...
async def start_product_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text("📂 প্রোডাক্টের ক্যাটাগরি বেছে নিন:", reply_markup=InlineKeyboardMarkup(keyboard))
...
def main():
    ...
    # Admin DB Restore Handler (Fixed Routing)
